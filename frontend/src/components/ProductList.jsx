...
import { useAuth } from "../context/AuthContext"; // ✅ เพิ่ม AuthContext
...
function ProductList() {
  ...
  // ... (useEffect และ fetchProducts เหมือนเดิม) ...
  useEffect(() => {
  ...
  const fetchProducts = (urlOverride) => {
    setLoading(true);
    let url =
      urlOverride || `http://localhost:8000/api/products/?sort=${sortOption}`;
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/api/products/${productId}/delete/`,
          {
            method: "DELETE",
            headers: {
        ...
        if (response.ok) {
          ...
          fetchProducts(null); // โหลดข้อมูลใหม่
        } else {
          Swal.fire("Error", "ไม่สามารถลบได้ (Unauthorized)", "error");
        }
      } catch (error) {
  ...
  // ... (SidebarFilter Component เหมือนเดิม) ...
  const SidebarFilter = () => (
    <div className="space-y-8">
      {/* ... (Code เดิมใน SidebarFilter) ... */}
      {/* 1. หมวดหมู่ */}
      <div>
        <h3 className="font-bold text-[#263A33] mb-4 text-lg">
          หมวดหมู่สินค้า
        </h3>
              ...
              <div
                className={`w-5 h-5 rounded border flex items-center justify-center transition ${
                  selectedCategory === cat
                    ? "bg-[#305949] border-[#305949]"
                    : "border-gray-300 group-hover:border-[#305949]"
                }`}
              >
                {selectedCategory === cat && (
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mb-3">
          <input
            ...
            className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#305949]"
          />
        ...
        <button
          ...
          className="w-full py-2 bg-[#305949] text-white text-sm font-bold rounded-lg hover:bg-[#234236] transition"
        >
          กรองราคา
        </button>
      </div>
            ...
            >
              <div
                className={`w-4 h-4 rounded-full border border-gray-300 ${
                  ratingFilter === star ? "bg-[#305949] border-[#305949]" : ""
                }`}
              ></div>
      ...
      <div>
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <input
            ...
            className="w-5 h-5 rounded text-[#305949] focus:ring-[#305949]"
          />
          <span className="text-sm text-gray-600">
            พร้อมส่งเท่านั้น (In Stock)
          </span>
        </label>
      </div>
    </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            <p className="text-xs text-gray-500 mb-1 font-medium">
              <Link to="/" className="hover:text-[#305949] transition-colors">
                หน้าหลัก
              </Link>
              <span className="mx-2">/</span>
              <span className="text-gray-800">สินค้าทั้งหมด</span>
            </p>
            <h1 className="text-3xl font-extrabold text-[#263A33]">
              🛍️ เลือกช้อปสินค้า
            </h1>
          </div>

          <div className="flex gap-3">
            {/* ✅ ปุ่มเพิ่มสินค้าสำหรับ Admin */}
            {user &&
              (user.role_code === "admin" ||
                user.role_code === "super_admin") && (
                <Link
                  to="/product/add"
                  className="hidden md:flex items-center justify-center gap-2 bg-[#305949] text-white px-4 py-2 rounded-lg shadow-sm font-bold hover:bg-[#234236] transition"
                >
                  + เพิ่มสินค้า
                </Link>
              )}
            <button
              ...
              className="md:hidden flex items-center justify-center gap-2 bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm text-[#263A33] font-bold"
            >
              ⚙️ ตัวกรอง
            </button>
          </div>
            ...
            <div className="flex justify-between items-center mb-6 bg-white p-3 px-5 rounded-xl shadow-sm border border-gray-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="text-sm font-bold text-[#263A33] bg-transparent border-none outline-none cursor-pointer hover:text-[#305949]"
                >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                {filteredProducts.map((product) => (
                  <div key={product.id} className="group bg-white rounded-2xl p-3 md:p-4 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 flex flex-col relative overflow-hidden">
                    {/* ✅ Admin Controls (แก้ไข/ลบ) */}
                    {user &&
                      (user.role_code === "admin" ||
                        user.role_code === "super_admin") && (
                        <div className="absolute top-2 right-2 z-20 flex gap-1 bg-white/90 p-1 rounded-lg shadow-sm backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link
                    ...
                    <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
                      {product.stock === 0 && (
                        <span className="bg-gray-800 text-white text-[10px] px-2 py-1 rounded-md font-bold uppercase">
                          Sold Out
                        </span>
                      )}
                      {product.rating >= 4.5 && (
                        <span className="bg-yellow-400 text-yellow-900 text-[10px] px-2 py-1 rounded-md font-bold">
                          ★ Top Rated
                        </span>
                      )}
                    </div>
            ...
            🛒
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <Link
                        ...
                        className="text-sm md:text-base font-bold text-[#263A33] hover:text-[#305949] line-clamp-2 mb-2 leading-tight"
                      >
                        {product.title}
                      </Link>
                      ...
                      <div className="mt-auto pt-2 border-t border-gray-50 flex items-center justify-between">
                        <div>
                          <span className="text-lg font-extrabold text-[#305949]">
            ...
            {!loading && (prevPage || nextPage) && (
              <div className="flex justify-center gap-3 mt-12">
                <button
                  disabled={!prevPage}
                  onClick={() => {
                    if (prevPage) fetchProducts(prevPage);
                    window.scrollTo(0, 0);
                  }}
                  className="px-5 py-2 bg-white border border-gray-200 rounded-full text-sm font-bold text-gray-600 hover:border-[#305949] hover:text-[#305949] disabled:opacity-50 disabled:hover:border-gray-200"
                >
                  ← หน้าก่อนหน้า
                </button>
                <button
                  ...
                  className="px-5 py-2 bg-[#305949] text-white rounded-full text-sm font-bold shadow-md hover:bg-[#234236] disabled:opacity-50"
                >
                  หน้าถัดไป →
                </button>
              </div>
