...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-300 ${isScrolled ? 'bg-white/80 backdrop-blur-xl shadow-sm py-3' : 'bg-[#F9F9F7] py-5'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-center sm:justify-between gap-4">
        
        <Link to="/" className="flex items-center gap-2 group">
          <div className="w-10 h-10 bg-gradient-to-br from-[#305949] to-[#234236] rounded-xl flex items-center justify-center text-white shadow-lg transform group-hover:rotate-6 transition-transform">
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          <Link to="/shop" className="hidden sm:flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-[#305949] transition-colors">
            <LayoutGrid size={18} /> สินค้าทั้งหมด
          </Link>
