        ...
        <div className="max-w-4xl mx-auto">
            ...
            {!user && (
                <div className="bg-white p-8 rounded-[2rem] shadow-sm mb-10 max-w-lg mx-auto">
                    <form onSubmit={handleSearch} className="flex gap-4">
                        <input type="tel" placeholder="กรอกเบอร์โทรศัพท์..." value={tel} onChange={(e) => setTel(e.target.value)} className="flex-1 bg-gray-50 px-6 py-3 rounded-xl border border-gray-200 outline-none focus:ring-2 focus:ring-[#305949]/20" required />
                        <button type="submit" className="bg-[#305949] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#234236] transition">ค้นหา</button>
                    </form>
                </div>
            )}
            ...
            {orders && (
                <div className="space-y-6">
                    {orders.length === 0 ? <div className="text-center text-gray-400 py-10">ไม่พบประวัติการสั่งซื้อ</div> : (
                        orders.map(order => (
                            <div key={order.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-white hover:shadow-md transition">
                                <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-4">
                                    <div><h3 className="font-bold text-lg text-gray-800">Order #{order.id}</h3><p className="text-xs text-gray-400">{new Date(order.created_at).toLocaleString()}</p></div>
                                ...
                                <div className="space-y-4">
                                    {order.items.map((item, i) => (
                                        <div key={i} className="flex gap-4 items-center">
                                            <div className="w-12 h-12 bg-gray-50 rounded-lg p-1 border border-gray-100"><img src={item.thumbnail || "https://placehold.co/100"} className="w-full h-full object-contain mix-blend-multiply" /></div>
                                            <div className="flex-1 text-sm"><p className="font-bold text-gray-700">{item.product_title}</p><p className="text-xs text-gray-400">x{item.quantity}</p></div>
